...
for nJetsBin in range(inputArguments.nJetsMin, 1 + inputArguments.nJetsMax):
    ...
    print("At nJets = " + str(nJetsBin) + ", nEntries_normBin = " + str(nEntries_normBin))
    # STHistograms[nJetsBin].Scale(nTargetEntries_normBin/nEntries_normBin, "width")
    normFactor = nTargetEntries_normBin/nEntries_normBin
    ...
    STHistogramsScaled[nJetsBin] = ROOT.TH1F("h_STDistributionScaled_{n}Jets".format(n=nJetsBin), "ST Distribution, nJets = {n};ST (GeV);A.U.".format(n = nJetsBin), n_STBins, array.array('d', STBoundaries))
    STHistogramsScaled[nJetsBin].Sumw2()
    for binIndex in range(1, 1+STHistogramsScaled[nJetsBin].GetXaxis().GetNbins()):
        STHistogramsScaled[nJetsBin].SetBinContent(binIndex, normFactor*STHistograms[nJetsBin].GetBinContent(binIndex))
        STHistogramsScaled[nJetsBin].SetBinError(binIndex, normFactor*STHistograms[nJetsBin].GetBinError(binIndex))
        if (STHistograms[inputArguments.nJetsNorm].GetBinContent(binIndex) > 0):
            ratio = normFactor*STHistograms[nJetsBin].GetBinContent(binIndex)/STHistograms[inputArguments.nJetsNorm].GetBinContent(binIndex)
            fractionalError_numerator = normFactor*STHistograms[nJetsBin].GetBinError(binIndex)/STHistograms[nJetsBin].GetBinContent(binIndex)
            fractionalError_denominator = STHistograms[inputArguments.nJetsNorm].GetBinError(binIndex)/STHistograms[inputArguments.nJetsNorm].GetBinContent(binIndex)
            ratioHistograms[nJetsBin].SetBinContent(binIndex, ratio)
            ratioHistograms[nJetsBin].SetBinError(binIndex, ratio*math.sqrt(fractionalError_numerator*fractionalError_numerator + fractionalError_denominator*fractionalError_denominator))
    STHistogramsScaled[nJetsBin].SetLineColor(histColors[nJetsBin])
    STHistogramsScaled[nJetsBin].Draw("HIST E1 SAME")
