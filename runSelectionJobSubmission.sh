...
if [ "${1}" == "data" -o "${1}" == "" ]; then
    ...
    for SELECTIONTYPE in ${SELECTIONTYPES[@]}; do
        for YEAR in ${YEARS[@]}; do
            ...
            set -x && ./submitEventSelectionJobs.py --inputFilesList inputFileList_data_DoubleEG_${YEAR}.txt --isMC false --photonSelectionType ${SELECTIONTYPE} --year ${YEAR} --JECUncertainty 0 --outputFilePrefix DoubleEG_${YEAR}_${SELECTIONTYPE} --outputDirectory selections/DoublePhoton/${SELECTIONTYPE} && set +x
        done
    done
fi
