...
def run_MC_chain(outputDirectory, eventProgenitor, crossSectionsFilePath, dataPrefix, outputPrefix, inputMCPathMain, inputDataPUSourceMain, inputHLTEfficienciesPathMain, PUWeightsOutputPathMain, integratedLuminosityMainString, inputMCPathsAux, inputDataPUSourcesAux, PUWeightsOutputPathsAux, inputHLTEfficienciesPathsAux, integratedLuminositiesAux, MCTemplatePath, getSignalContaminationOutsideSidebands, optional_identifier, isDryRun):
...
def produce_ancillary_plots_control(outputDirectory, eventProgenitor, path_data_expectedNEvents, path_data_observedNEvents, path_MC_weightedNEvents, path_dataSystematics, optional_identifier, isDryRun):
    for outputSubdirectory in ["dataEventHistograms", "dataSystematics", "publicationPlots"]:
        os.system("mkdir -p {oD}/{oS}".format(oD=outputDirectory, oS=outputSubdirectory))
    command_controlSTDistributions_dataAndSignal = "./plotSTDistributionsWithErrors.py --eventProgenitor {eP} --path_data_expectedNEvents {pDENE} --path_data_observedNEvents {pDONE} --path_MC_weightedNEvents {pMCWNE} --path_dataSystematics {pDS} --outputDirectory {oD}/publicationPlots/ --outputFilePrefix {oFP} --plotObservedData".format(eP=eventProgenitor, pDENE=path_data_expectedNEvents, pDONE=path_data_observedNEvents, pMCWNE=path_MC_weightedNEvents, pDS=path_dataSystematics, oD=outputDirectory, oFP="STDistributions_{eP}_control".format(eP=eventProgenitor))
    execute_in_env(command_controlSTDistributions_dataAndSignal, optional_identifier, isDryRun)

def produce_ancillary_plots_signal(outputDirectory, eventProgenitor, signalType, path_data_expectedNEvents, path_data_observedNEvents, path_MC_weightedNEvents, path_dataSystematics, runUnblinded, optional_identifier, isDryRun):
    for outputSubdirectory in ["dataEventHistograms", "dataSystematics", "publicationPlots"]:
        os.system("mkdir -p {oD}/{oS}".format(oD=outputDirectory, oS=outputSubdirectory))
    command_signalSTDistributions_dataAndSignal = "./plotSTDistributionsWithErrors.py --eventProgenitor {eP} --path_data_expectedNEvents {pDENE} --path_data_observedNEvents {pDONE} --path_MC_weightedNEvents {pMCWNE} --path_dataSystematics {pDS} --outputDirectory {oD}/publicationPlots/ --outputFilePrefix {oFP}".format(eP=eventProgenitor, pDENE=path_data_expectedNEvents, pDONE=path_data_observedNEvents, pMCWNE=path_MC_weightedNEvents, pDS=path_dataSystematics, oD=outputDirectory, oFP="STDistributions_{eP}_{sT}".format(eP=eventProgenitor, sT=signalType))
    if runUnblinded:
...
for step in runSequence:
    if (step == "data"):
        run_data_step(outputDirectory="{aR}/analysis{oI}".format(aR=stealthEnv.analysisRoot, oI=optional_identifier), inputFilesList="{eP}/{sER}/selections/combined_DoublePhoton{sS}/merged_selection_data_{yP}_control.root".format(eP=stealthEnv.EOSPrefix, sER=stealthEnv.stealthEOSRoot, sS=selection_suffix, yP=yearPattern), outputPrefix="control", analyzeSignalBins=True, optional_identifier=optional_identifier, isDryRun=inputArguments.isDryRun)
        ...
        for eventProgenitor in eventProgenitors:
            ...
            for signalType in (list_signalTypes + ["control"]):
                ...
                if (inputArguments.year == "all"):
                    ...
                    HLTEfficienciesPathMain = "{eP}/{sER}/HLTEfficiencies/HLTEfficiencies_{sT}.root".format(eP=stealthEnv.EOSPrefix, sER=stealthEnv.stealthEOSRoot, sT=signalType)
                    ...
                    HLTEfficienciesPathsAux = ["{eP}/{sER}/HLTEfficiencies/HLTEfficiencies_{sT}.root".format(eP=stealthEnv.EOSPrefix, sER=stealthEnv.stealthEOSRoot, sT=signalType), "{eP}/{sER}/HLTEfficiencies/HLTEfficiencies_{sT}.root".format(eP=stealthEnv.EOSPrefix, sER=stealthEnv.stealthEOSRoot, sT=signalType)]
                    ...
                    HLTEfficienciesPathMain = "{eP}/{sER}/HLTEfficiencies/HLTEfficiencies_{sT}.root"
                ...
                run_MC_chain(outputDirectory="{aR}/analysis{oI}".format(aR=stealthEnv.analysisRoot, oI=optional_identifier), eventProgenitor=eventProgenitor, crossSectionsFilePath=crossSectionsPath, dataPrefix="{sT}".format(sT=signalType), outputPrefix="MC_stealth_{eP}_{y}_{sT}".format(eP=eventProgenitor, y=inputArguments.year, sT=signalType), inputMCPathMain=MCPathMain, inputDataPUSourceMain=dataPUSourceMain, inputHLTEfficienciesPathMain=HLTEfficienciesPathMain, PUWeightsOutputPathMain=PUWeightsOutputPathMain, integratedLuminosityMainString=lumiMain, inputMCPathsAux=MCPathsAux, inputDataPUSourcesAux=dataPUSourcesAux, PUWeightsOutputPathsAux=PUWeightsOutputPathsAux, inputHLTEfficienciesPathsAux=HLTEfficienciesPathsAux, integratedLuminositiesAux=lumisAux, MCTemplatePath=MCTemplatesForProgenitor[eventProgenitor], getSignalContaminationOutsideSidebands=get_signalContamination_outside_sidebands, optional_identifier=optional_identifier, isDryRun=inputArguments.isDryRun)
                # def run_MC_chain(outputDirectory, eventProgenitor, crossSectionsFilePath, dataPrefix, outputPrefix, inputMCPathMain, inputDataPUSourceMain, inputHLTEfficienciesPathMain, PUWeightsOutputPathMain, integratedLuminosityMainString, inputMCPathsAux, inputDataPUSourcesAux, PUWeightsOutputPathsAux, inputHLTEfficienciesPathsAux, integratedLuminositiesAux, MCTemplatePath, getSignalContaminationOutsideSidebands, optional_identifier, isDryRun):
    elif (step == "combine"):
        ...
        for eventProgenitor in eventProgenitors:
            crossSectionsPath = crossSectionsForProgenitor[eventProgenitor]
            produce_ancillary_plots_control(outputDirectory="{aR}/analysis{oI}".format(aR=stealthEnv.analysisRoot, oI=optional_identifier), eventProgenitor=eventProgenitor, path_data_expectedNEvents="{aR}/analysis{oI}/dataSystematics/control_eventCounters.dat".format(aR=stealthEnv.analysisRoot, oI=optional_identifier), path_data_observedNEvents="{aR}/analysis{oI}/dataSystematics/control_observedEventCounters.dat".format(aR=stealthEnv.analysisRoot, oI=optional_identifier), path_MC_weightedNEvents="{aR}/analysis{oI}/MCEventHistograms/MC_stealth_{eP}_{y}_control_savedObjects.root".format(aR=stealthEnv.analysisRoot, oI=optional_identifier, eP=eventProgenitor, y=inputArguments.year), path_dataSystematics="{aR}/analysis{oI}/dataSystematics/control_dataSystematics.dat".format(aR=stealthEnv.analysisRoot, oI=optional_identifier), optional_identifier=optional_identifier, isDryRun=inputArguments.isDryRun)
            for signalType in list_signalTypes:
                produce_ancillary_plots_signal(outputDirectory="{aR}/analysis{oI}".format(aR=stealthEnv.analysisRoot, oI=optional_identifier), eventProgenitor=eventProgenitor, signalType=signalType, path_data_expectedNEvents="{aR}/analysis{oI}/dataSystematics/{sT}_eventCounters.dat".format(aR=stealthEnv.analysisRoot, oI=optional_identifier, sT=signalType), path_data_observedNEvents="{aR}/analysis{oI}/dataSystematics/{sT}_observedEventCounters.dat".format(aR=stealthEnv.analysisRoot, oI=optional_identifier, sT=signalType), path_MC_weightedNEvents="{aR}/analysis{oI}/MCEventHistograms/MC_stealth_{eP}_{y}_{sT}_savedObjects.root".format(aR=stealthEnv.analysisRoot, oI=optional_identifier, eP=eventProgenitor, y=inputArguments.year, sT=signalType), path_dataSystematics="{aR}/analysis{oI}/dataSystematics/{sT}_dataSystematics.dat".format(aR=stealthEnv.analysisRoot, oI=optional_identifier, sT=signalType), runUnblinded=inputArguments.runUnblinded, optional_identifier=optional_identifier, isDryRun=inputArguments.isDryRun)
    ...
    #         lumisAux = []
    #         if (inputArguments.year == "all"):
    #             MCPathMain = "{eP}/{sER}/selections/combined_DoublePhoton{sS}/merged_selection_MC_stealth_{tD}_2017_control.root".format(eP=stealthEnv.EOSPrefix, tD=tDesignationsForProgenitor[eventProgenitor], sER=stealthEnv.stealthEOSRoot, sS=selection_suffix)
    #             lumiMain = integrated_lumi_strings["2017"]
    #             MCPathsAux = ["{eP}/{sER}/selections/combined_DoublePhoton{sS}/merged_selection_MC_stealth_{tD}_2016_control.root".format(eP=stealthEnv.EOSPrefix, tD=tDesignationsForProgenitor[eventProgenitor], sER=stealthEnv.stealthEOSRoot, sS=selection_suffix), "{eP}/{sER}/selections/combined_DoublePhoton{sS}/merged_selection_MC_stealth_{tD}_2018_control.root".format(eP=stealthEnv.EOSPrefix, tD=tDesignationsForProgenitor[eventProgenitor], sER=stealthEnv.stealthEOSRoot, sS=selection_suffix)]
    #             lumisAux = [integrated_lumi_strings["2016"], integrated_lumi_strings["2018"]]
    #         else:
    #             MCPathMain = "{eP}/{sER}/selections/combined_DoublePhoton{sS}/merged_selection_MC_stealth_{tD}_{y}_control.root".format(eP=stealthEnv.EOSPrefix, tD=tDesignationsForProgenitor[eventProgenitor], sER=stealthEnv.stealthEOSRoot, sS=selection_suffix, y=inputArguments.year)
    #             lumiMain = integrated_lumi_strings[inputArguments.year]
