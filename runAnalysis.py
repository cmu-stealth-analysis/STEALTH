...
for step in runSequence:
    if (step == "data"):
        for signalType in (list_signalTypes + ["control"]):
            produce_STComparisons(dataPath="{eP}/{sER}/selections/combined_DoublePhoton{sS}/merged_selection_MC_GJet_2017_{sT}.root".format(eP=stealthEnv.EOSPrefix, sER=stealthEnv.stealthEOSRoot, sS=selection_suffix, sT=signalType), outputFilePrefix_STComparisons="{sT}_GJet_STComparisons".format(sT=signalType), analyzeSignalBins=True, useWeights=True)
