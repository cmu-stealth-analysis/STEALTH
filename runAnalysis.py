...
def produce_STComparisons(outputDirectory, controlDataPath, outputFileName_STComparisons, optional_identifier):
...
for step in runSequence:
    if (step == "data"):
        ...
        produce_STComparisons(outputDirectory="{aR}/analysis{oI}".format(aR=stealthEnv.analysisRoot, oI=optional_identifier), controlDataPath="{eP}/{sER}/selections/combined_DoublePhoton{sS}/merged_selection_data_{yP}_control_fakefake.root".format(eP=stealthEnv.EOSPrefix, sER=stealthEnv.stealthEOSRoot, sS=selection_suffix, yP=yearPattern), outputFileName_STComparisons="control_STComparison.png", optional_identifier=optional_identifier)
        for eventProgenitor in eventProgenitors:
            crossSectionsPath = crossSectionsForProgenitor[eventProgenitor]
