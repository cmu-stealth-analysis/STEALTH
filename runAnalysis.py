...
def plot_limits(outputDirectory, combineResultsDirectory, MCTemplatePath, runUnblinded, optional_identifier):
    ...
    command_plotLimits = "condor_q && ./plotLimits.py --MCTemplatePath {MTP} --combineResultsDirectory {cRD} --combineOutputPrefix fullChain --outputDirectory_rawOutput limits --outputDirectory_plots {oD}/publicationPlots --outputSuffix fullChain".format(MTP=MCTemplatePath, cRD=combineResultsDirectory, oD=outputDirectory)
