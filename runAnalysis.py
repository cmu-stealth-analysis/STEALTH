...
def run_combine_chain(outputDirectory, outputPrefix, MCTemplatePath, luminosity_uncertainty, runUnblinded, optional_identifier):
    ...
    command_submitCombineJobs = ("./submitCombineToolJobs.py --dataCardsDirectory {oD} --dataCardsPrefix {oP} --outputDirectory {eP}/{sER}/combineToolOutputs/".format(oD=outputDirectory, oP=outputPrefix, eP=stealthEnv.EOSPrefix, sER=stealthEnv.stealthEOSRoot))
...
for step in runSequence:
    if (step == "data"):
        ...
        run_data_step(outputDirectory="analysis{oI}".format(oI=optional_identifier), inputFilesList="{eP}/{sER}/selections/combined_DoublePhoton{oI}/merged_selection_data_2017_signal.root".format(eP=stealthEnv.EOSPrefix, sER=stealthEnv.stealthEOSRoot, oI=optional_identifier), outputPrefix="signal", isSignal=True, runUnblinded=options.runUnblinded, optional_identifier=optional_identifier)
    elif (step == "MC"):
        ...
        run_combine_chain(outputDirectory="analysis{oI}".format(oI=optional_identifier), outputPrefix="fullChain", MCTemplatePath="{eP}/{sER}/MCGeneratedMasses/MCGeneratedMasses/MCGeneratedMasses_mc_Fall17_stealth_t5Wg_savedObjects.root".format(eP=stealthEnv.EOSPrefix, sER=stealthEnv.stealthEOSRoot), luminosity_uncertainty=lumi_uncertainty, runUnblinded=options.runUnblinded, optional_identifier=optional_identifier)
