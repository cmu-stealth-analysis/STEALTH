...
for step in runSequence:
    if (step == "data"):
        ...
        for signalType in ["signal", "signal_loose"]:
            run_data_step(outputDirectory="analysis{oI}".format(oI=optional_identifier), inputFilesList="{eP}/{sER}/selections/combined_DoublePhoton{sS}/merged_selection_data_{yP}_{sT}.root".format(eP=stealthEnv.EOSPrefix, sER=stealthEnv.stealthEOSRoot, sS=selection_suffix, yP=yearPattern, sT=signalType), outputPrefix="{sT}".format(sT=signalType), getSystematics=True, getSTScalingSystematics=False, analyzeSignalBins=inputArguments.runUnblinded, optional_identifier=optional_identifier)
    elif (step == "MC"):
        for signalType in ["signal", "signal_loose"]:
            MCPathMain = ""
            lumiMain = ""
            ...
            run_MC_chain(outputDirectory="analysis{oI}".format(oI=optional_identifier), dataPrefix="{sT}".format(sT=signalType), outputPrefix="MC_stealth_t5_{y}_{sT}".format(y=inputArguments.year, sT=signalType), inputMCPathMain=MCPathMain, integratedLuminosityMainString=lumiMain, inputMCPathsAux=MCPathsAux, integratedLuminositiesAux=lumisAux, MCTemplatePath="{eP}/{sER}/MCGeneratedMasses/MCGeneratedMasses/MCGeneratedMasses_mc_Fall17_stealth_t5Wg_savedObjects.root".format(eP=stealthEnv.EOSPrefix, sER=stealthEnv.stealthEOSRoot), getSignalContaminationOutsideSidebands=False, optional_identifier=optional_identifier)
    elif (step == "combine"):
