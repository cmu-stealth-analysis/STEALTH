...
def get_commands_ancillary_plots_control(path_data_expectedNEvents, path_data_observedNEvents, path_data_adjustments, path_MC_weightedNEvents_gluino, path_MC_weightedNEvents_squark, path_systematics_nominal, path_systematics_dataMCDiscrepancy, nJetsBin):
    ...
    command_controlSTDistributions_dataAndSignal = "./plotSTDistributionsWithErrors.py --path_data_expectedNEvents {pDENE} --path_data_observedNEvents {pDONE} --path_data_adjustments {pDA} --path_MC_weightedNEvents_gluino {pMCWNEg} --path_MC_weightedNEvents_squark {pMCWNEs} --path_systematics_nominal {pSN} --path_systematics_dataMCDiscrepancy {pSDMCD} --outputDirectory {aOD}/publicationPlots/ --outputFilePrefix {oFP} --nJetsBin {n} --plotObservedData --suppressSignal".format(pDENE=path_data_expectedNEvents, pDONE=path_data_observedNEvents, pDA=path_data_adjustments, pMCWNEg=path_MC_weightedNEvents_gluino, pMCWNEs=path_MC_weightedNEvents_squark, pSN=path_systematics_nominal, pSDMCD=path_systematics_dataMCDiscrepancy, aOD=analysisOutputDirectory, oFP="STDistributions_control", n=nJetsBin)
...
def get_commands_ancillary_plots_signal(signalType, path_data_expectedNEvents, path_data_observedNEvents, path_data_adjustments, path_MC_weightedNEvents_gluino, path_MC_weightedNEvents_squark, path_systematics_nominal, path_systematics_dataMCDiscrepancy, nJetsBin, run_unblinded):
    ...
    command_signalSTDistributions_dataAndSignal = "./plotSTDistributionsWithErrors.py --path_data_expectedNEvents {pDENE} --path_data_observedNEvents {pDONE} --path_data_adjustments {pDA} --path_MC_weightedNEvents_gluino {pMCWNEg} --path_MC_weightedNEvents_squark {pMCWNEs} --path_systematics_nominal {pSN} --path_systematics_dataMCDiscrepancy {pSDMCD} --outputDirectory {aOD}/publicationPlots/ --nJetsBin {n}".format(pDENE=path_data_expectedNEvents, pDONE=path_data_observedNEvents, pDA=path_data_adjustments, pMCWNEg=path_MC_weightedNEvents_gluino, pMCWNEs=path_MC_weightedNEvents_squark, pSN=path_systematics_nominal, pSDMCD=path_systematics_dataMCDiscrepancy, aOD=analysisOutputDirectory, n=nJetsBin)
    if run_unblinded:
        command_signalSTDistributions_dataAndSignal += " --plotObservedData --path_fitDiagnostics {p} --bin_label_abbreviation {bla} --outputFilePrefix {oFP}".format(p="{eP}/{aEOD}/fitDiagnostics.root".format(eP=stealthEnv.EOSPrefix, aEOD=analysisEOSOutputDirectory), bla=binLabelAbbreviations[signalType], oFP="STDistributions_postFit_{sT}".format(sT=signalType))
    else:
