...
def checkAndEstablishLock(): # Make sure that at most one instance is running at a time
    ...
    if (os.path.isfile("{aLD}/analysisRunning.lock".format(aLD=analysisLogsDirectory))):
        ...
        for outputSubdirectory in ["cutFlow", "dataEventHistograms", "dataSystematics", "fits_doublephoton", "HLTEfficiencies", "MCEventHistograms", "MCSystematics", "signalContamination", "publicationPlots", "limits", "statisticsChecks", "analysisLogs"]:
...
lumi_uncertainty = 0.016 # https://twiki.cern.ch/twiki/bin/view/CMS/TWikiLUM
...
total_lumi = lumi_values_raw_json["2016"] + lumi_values_raw_json["2017"] + lumi_values_raw_json["2018"]
...
def run_combine_chain(eventProgenitor, path_dataSystematics_signal, path_dataSystematics_signal_loose, # path_dataSystematics_control,
                      ...
                      outputPrefix):
    command_submitCombineJobs = ("./submitCombineToolJobs.py --dataCardsPrefix {oP} --outputDirectory {eP}/{cREOSOD}/ --eventProgenitor {eP2} --MCTemplatePath {MTP} --minNeutralinoMass {mNM} --eventProgenitorMassOffset {ePMO} --minMassDifference {mMD} --crossSectionsFileName {cSFP} --path_dataSystematics_signal {pDSS} --path_dataSystematics_signal_loose {pDSSL} --path_MCShapeAdjustment_signal {pSAS} --path_MCShapeAdjustment_signal_loose {pSASL} --paths_bkgCompositionSystematic {pBCS} --path_dataObservedEventCounters_signal {pDOECS} --path_dataObservedEventCounters_signal_loose {pDOECSL} --path_dataExpectedEventCounters_signal {pDEECS} --path_dataExpectedEventCounters_signal_loose {pDEECSL} --MCHistogramsSignal {EAA}/{pS}_savedObjects.root --MCHistogramsSignalLoose {EAA}/{pSL}_savedObjects.root --MCUncertaintiesSignal {EAA}/{pS}_MCUncertainties_savedObjects.root --MCUncertaintiesSignalLoose {EAA}/{pSL}_MCUncertainties_savedObjects.root --mismodelingUncertainty {mU} --totalLumi {tL} --luminosityUncertainty {lU} --EOSAnalysisArea {EAA}".format(oP=outputPrefix, eP=stealthEnv.EOSPrefix, sER=stealthEnv.stealthEOSRoot, MTP=MCTemplatesForProgenitor[eventProgenitor], mNM=minNeutralinoMassToPlot[eventProgenitor], ePMO=eventProgenitorMassOffsets[eventProgenitor], mMD=minMassDifferences[eventProgenitor], eP2=eventProgenitor, cREOSOD=combineResultsEOSOutputDirectory, cSFP=crossSectionsForProgenitor[eventProgenitor], pDSS=path_dataSystematics_signal, pDSSL=path_dataSystematics_signal_loose, pSAS=path_MCShapeAdjustment_signal, pSASL=path_MCShapeAdjustment_signal_loose, pBCS=paths_bkgCompositionSystematic, pDOECS=path_dataObservedEventCounters_signal, pDOECSL=path_dataObservedEventCounters_signal_loose, pDEECS=path_dataExpectedEventCounters_signal, pDEECSL=path_dataExpectedEventCounters_signal_loose, pS=MCPrefix_signal, pSL=MCPrefix_signal_loose, mU=mismodeling_uncertainty, tL=total_lumi, lU=lumi_uncertainty, EAA="{eP}/{aEOD}".format(eP=stealthEnv.EOSPrefix, aEOD=analysisEOSOutputDirectory)))
...
def get_commands_plot_limits(eventProgenitor):
    ...
    # With just +/-1 sigma expected limit contours
    ...
    if inputArguments.runUnblinded:
        ...
        commands_plot_limits.append(command_plotLimits)
    # with +/- 2 sigma expected limit contours in addition to +/-1 sigma expected limit contours
    command_plotLimits = "./plotLimits.py --crossSectionsFile {cSFP} --MCTemplatePath {MTP} --eventProgenitor {eP2} --combineResultsDirectory {eP}/{cREOSOD} --combineOutputPrefix {eP2} --outputDirectory_rawOutput {aOD}/limits --outputDirectory_plots {aOD}/publicationPlots --outputSuffix {eP2}_pm2SigmaExp --minNeutralinoMass {mNM} --eventProgenitorMassOffset {ePMO} --minMassDifference {mMD} --selectionsList {sS} --signalContaminationSource_signal {sCSS} --signalContaminationSource_signal_loose {sCSSL} --signalContaminationMonitor_source_folder_eos {sCMSF} --plot2SigmaExp".format(eP=stealthEnv.EOSPrefix, MTP=MCTemplatesForProgenitor[eventProgenitor], cREOSOD=combineResultsEOSOutputDirectory, eP2=eventProgenitor, cSFP=crossSectionsForProgenitor[eventProgenitor], aOD=analysisOutputDirectory, mNM=minNeutralinoMassToPlot[eventProgenitor], ePMO=eventProgenitorMassOffsets[eventProgenitor], mMD=minMassDifferences[eventProgenitor], sS=selectionsString, sCSS="{eP}/{aEOD}/MC_stealth_{eP2}_all_signal_MCUncertainties_savedObjects.root".format(eP=stealthEnv.EOSPrefix, eP2=eventProgenitor, aEOD=analysisEOSOutputDirectory), sCSSL="{eP}/{aEOD}/MC_stealth_{eP2}_all_signal_loose_MCUncertainties_savedObjects.root".format(eP=stealthEnv.EOSPrefix, eP2=eventProgenitor, aEOD=analysisEOSOutputDirectory), sCMSF="{aEOD}/signalContaminationMonitor".format(aEOD=analysisEOSOutputDirectory))
    commands_plot_limits.append(command_plotLimits)
    if inputArguments.runUnblinded:
        command_plotLimits += " --plotObserved"
        commands_plot_limits.append(command_plotLimits)
...
# ./runAnalysis.py --noMCNorms --optionalIdentifier "HigherJetPTThreshold" --selectionSuffix "HigherJetPTThreshold" --noLooseSignal --runUnblinded
# OR
# ./runAnalysis.py --optionalIdentifier "HigherJetPTThreshold" --selectionSuffix "HigherJetPTThreshold" --noLooseSignal --runUnblinded --chain "data" && ./runAnalysis.py --noMCNorms --optionalIdentifier "HigherJetPTThreshold" --selectionSuffix "HigherJetPTThreshold" --noLooseSignal --runUnblinded --chain "BKGMC" && ./runAnalysis.py --optionalIdentifier "HigherJetPTThreshold" --selectionSuffix "HigherJetPTThreshold" --noLooseSignal --runUnblinded --chain "MC" && ./runAnalysis.py --optionalIdentifier "HigherJetPTThreshold" --selectionSuffix "HigherJetPTThreshold" --noLooseSignal --runUnblinded --chain "combine" && ./runAnalysis.py --optionalIdentifier "HigherJetPTThreshold" --selectionSuffix "HigherJetPTThreshold" --noLooseSignal --runUnblinded --chain "ancillaryPlots"
# ./runAnalysis.py --optionalIdentifier "HigherJetPTThreshold" --selectionSuffix "HigherJetPTThreshold" --noLooseSignal --runUnblinded --chain "observations" && ./runAnalysis.py --optionalIdentifier "HigherJetPTThreshold" --selectionSuffix "HigherJetPTThreshold" --noLooseSignal --runUnblinded --chain "limits" && ./runAnalysis.py --optionalIdentifier "HigherJetPTThreshold" --selectionSuffix "HigherJetPTThreshold" --noLooseSignal --runUnblinded --chain "statisticsChecks"
