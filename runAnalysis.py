...
def run_combine_chain(outputDirectory, combineResultsDirectory, prefix_MCChainStep_signal, prefix_MCChainStep_signal_loose, outputPrefix, MCTemplatePath, luminosity_uncertainty, runUnblinded, optional_identifier):
...
for step in runSequence:
    if (step == "data"):
        ...
        for signalType in ["signal", "signal_loose"]:
        ...
        produce_ancillary_plots(outputDirectory="{aR}/analysis{oI}".format(aR=stealthEnv.analysisRoot, oI=optional_identifier), controlDataPath="{eP}/{sER}/selections/combined_DoublePhoton{sS}/merged_selection_data_{yP}_control_fakefake.root".format(eP=stealthEnv.EOSPrefix, sER=stealthEnv.stealthEOSRoot, sS=selection_suffix, yP=yearPattern), outputFileName_STComparisons="control_STComparison.png", path_data_expectedNEvents_control="{aR}/analysis{oI}/dataSystematics/control_eventCounters.dat".format(aR=stealthEnv.analysisRoot, oI=optional_identifier), path_data_observedNEvents_control="{aR}/analysis{oI}/dataSystematics/control_observedEventCounters.dat".format(aR=stealthEnv.analysisRoot, oI=optional_identifier), path_MC_weightedNEvents_control="{aR}/analysis{oI}/MCEventHistograms/MC_stealth_t5_{y}_control_savedObjects.root".format(aR=stealthEnv.analysisRoot, oI=optional_identifier, y=inputArguments.year), path_dataSystematics_control="{aR}/analysis{oI}/dataSystematics/control_dataSystematics.dat".format(aR=stealthEnv.analysisRoot, oI=optional_identifier), path_STScalingSystematics="{aR}/analysis{oI}/dataSystematics/control_dataSystematics_sTScaling.dat".format(aR=stealthEnv.analysisRoot, oI=optional_identifier), path_data_expectedNEvents_signal="{aR}/analysis{oI}/dataSystematics/signal_eventCounters.dat".format(aR=stealthEnv.analysisRoot, oI=optional_identifier), path_data_observedNEvents_signal="{aR}/analysis{oI}/dataSystematics/signal_observedEventCounters.dat".format(aR=stealthEnv.analysisRoot, oI=optional_identifier), path_MC_weightedNEvents_signal="{aR}/analysis{oI}/MCEventHistograms/MC_stealth_t5_{y}_signal_savedObjects.root".format(aR=stealthEnv.analysisRoot, oI=optional_identifier, y=inputArguments.year), path_dataSystematics_signal="{aR}/analysis{oI}/dataSystematics/signal_dataSystematics.dat".format(aR=stealthEnv.analysisRoot, oI=optional_identifier), runUnblinded=inputArguments.runUnblinded, optional_identifier=optional_identifier)
