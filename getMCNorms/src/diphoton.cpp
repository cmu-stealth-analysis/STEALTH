...
std::string get_output_st_distribution_name(const int & nJetsBin, const bool & STSwitch_fineBinned, const bool & STSwitch_withLowST) {
  std::string fineBinned = "";
  if (STSwitch_fineBinned) fineBinned = "fineBinned_";
  std::string withLowST = "";
  if (STSwitch_withLowST) withLowST = "withLowST_";
  return (std::string("ST_") + fineBinned + withLowST + std::to_string(nJetsBin) + std::string("JetsBin"));
}

void initialize_output_th1s_map(std::map<std::string, TH1D> & output_th1s, const STRegionsStruct & STRegions, const STRegionsStruct & STRegionsFineBinned, const STRegionsStruct & STRegionsWithLowST) {
  std::string hname;
  ...
  for (int nJetsBin = 2; nJetsBin <= 6; ++nJetsBin) {
    hname = get_output_st_distribution_name(nJetsBin, false, false);
    assert(output_th1s.find(hname) == output_th1s.end());
    ...
    output_th1s[hname].Sumw2();
    hname = get_output_st_distribution_name(nJetsBin, true, false);
    assert(output_th1s.find(hname) == output_th1s.end());
    ...
    output_th1s[hname].Sumw2();
    hname = get_output_st_distribution_name(nJetsBin, false, true);
    assert(output_th1s.find(hname) == output_th1s.end());
    output_th1s[hname] = TH1D(hname.c_str(), ("ST, " + std::to_string(nJetsBin) + " Jets").c_str(), (STRegionsWithLowST.STBoundaries.size()-1), &(STRegionsWithLowST.STBoundaries.at(0)));
    output_th1s[hname].Sumw2();
  }
}
...
bool passes_selection3(eventDataStruct & event_data, const double & STNormRangeMin, const double & STFineBinnedNormRangeMin, const double & STWithLowSTNormRangeMin) {
  if (event_data.nJetsDR < 2) return false;
  if (((event_data.evtST < STNormRangeMin) && (event_data.evtST < STFineBinnedNormRangeMin)) && (event_data.evtST < STWithLowSTNormRangeMin)) return false;
  return true;
}
...
void fill_histograms3(eventDataStruct & event_data, std::map<std::string, TH1D> & output_th1s, const bool & addMCWeights, const double & STNormRangeMin, const double & STFineBinnedNormRangeMin, const double & STWithLowSTNormRangeMin) {
  ...
  if (event_data.evtST >= STWithLowSTNormRangeMin) {
    hname = get_output_st_distribution_name(nJetsBin, false, true);
    bin_width = (output_th1s.at(hname)).GetXaxis()->GetBinWidth((output_th1s.at(hname)).GetXaxis()->FindFixBin(event_data.evtST));
    weight = 1.0/bin_width;
    if (addMCWeights) {
...
void loop_over_chain_events(TChain * inputChain, eventDataStruct & event_data, std::map<std::string, TH1D> & output_th1s, const bool & addMCWeights, const bool & doStealthMCSelection, const STRegionsStruct & STRegions, const STRegionsStruct & STRegionsFineBinned, const STRegionsStruct & STRegionsWithLowST) {
  ...
  for (Long64_t entryIndex = 0; entryIndex < nEntries; ++entryIndex) {
    ...
    if (passes_selection3(event_data, STRegions.STNormRangeMin, STRegionsFineBinned.STNormRangeMin, STRegionsWithLowST.STNormRangeMin)) fill_histograms3(event_data, output_th1s, addMCWeights, STRegions.STNormRangeMin, STRegionsFineBinned.STNormRangeMin, STRegionsWithLowST.STNormRangeMin);
  }
