...
if [[ "$BASEHOSTNAME" =~ ^lxplus[0-9]{3,4}$ ]]; then
    ...
    cd /afs/cern.ch/user/t/tmudholk/public/research/stealth/from_michael/StealthProduction/CMSSW_9_4_13/src && set +x && eval `scramv1 runtime -sh` && export PYTHONPATH=/afs/cern.ch/user/t/tmudholk/public/tmPyUtils:$PYTHONPATH && set -x
    ...
    set +x && source /cvmfs/cms.cern.ch/cmsset_default.sh && set -x
    export SCRAM_ARCH=slc6_amd64_gcc630
    cd /uscms/home/tmudholk/private/stealth/cmssw/CMSSW_9_4_13/src && set +x && eval `scramv1 runtime -sh` && export PYTHONPATH=/afs/cern.ch/user/t/tmudholk/public/tmPyUtils:$PYTHONPATH && set -x
    echo "Set default CMSSW_BASE = ${CMSSW_BASE}"
    cd ${WORKDIR}
else
