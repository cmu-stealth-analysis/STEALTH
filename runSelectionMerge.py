...
for inputSelectionToRun in (inputArguments.selectionsToRun.split(",")):
    if (inputSelectionToRun == "data"):
        ...
        for selectionRegion in ["signal", "signal_loose", "control_fakefake"]:
            mergeStep2FilePath = "fileLists/inputFileList_step2Merge_MC_GJet{aJS}{eVS}_2018{oI}_{r}.txt".format(oI=optional_identifier, aJS=allJetsString, eVS=electronVetoString, r=selectionRegion)
        ...
        for selectionRegion in ["control_singlemedium", "control_singleloose", "control_singlefake"]:
            mergeStep2FilePath = "fileLists/inputFileList_step2Merge_MC_GJet_singlephoton{aJS}{eVS}_2018{oI}_{r}.txt".format(oI=optional_identifier, aJS=allJetsString, eVS=electronVetoString, r=selectionRegion)
            os.system("rm -f {mS2FP} && touch {mS2FP}".format(mS2FP=mergeStep2FilePath))
...
for selectionType in selectionTypesToRun:
    ...
    for year in yearsToRun:
        ...
        if (bool(re.match(r"^MC_GJet[0-9]*$", selectionType))):
            if (year != 2018): # The only reason we need these is to calculate scaling systematics
                mergeStatistics = False
        ...
        for selectionRegion in ["signal", "signal_loose", "control_fakefake", "control_singlemedium", "control_singleloose", "control_singlefake"]:
            ...
            if (((bool(re.match(r"^MC_GJet[0-9]*$", selectionType))) or (bool(re.match(r"^MC_GJet_singlephoton[0-9]*$", selectionType)))) and (year != 2018)): mergeSelection = False # The only reason we need these is to calculate scaling systematics
        ...
        for selectionRegion in ["signal", "signal_loose", "control_fakefake", "control_singlemedium", "control_singleloose", "control_singlefake"]:
            ...
            if ((selectionRegion == "control_singlemedium") or (selectionRegion == "control_singleloose") or (selectionRegion == "control_singlefake")):
                ...
                if ((selectionType == "MC_GJet_singlephoton") or
                    (selectionType == "MC_QCD_singlephoton")):
                    mergeSelection = False
            if (((selectionType == "MC_GJet") or (selectionType == "MC_GJet_singlephoton")) and (year != 2018)): mergeSelection = False
            if (((selectionType == "MC_QCD") or (selectionType == "MC_QCD_singlephoton")) and (year != 2017)): mergeSelection = False
            if not(mergeSelection): continue
            # mergeStep2FilePath = "fileLists/inputFileList_step2Merge_{t}{aJS}{eVS}_2018{oI}_{r}.txt".format(t=selectionType, oI=optional_identifier, aJS=allJetsString, eVS=electronVetoString, r=selectionRegion)
