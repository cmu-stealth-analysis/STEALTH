...
# First read in observedNEvents for the control sample, and calculate the fraction of expected to observed events (while analysis is still blinded, we use these ratios to calculate fake observed data given QCD expectations in the signal regions)
observedToExpectedRatio_control = {}
observedNEvents_control = get_dict_nEvents(inputPath=inputDataFilePaths[signalType]["observations"], localSignalLabels=localSignalBinLabels, inputPrefix="observedNEvents")
for localLabel in localSignalBinLabels:
    globalLabel = dict_localToGlobalBinLabels["control"][localLabel]
    observedToExpectedRatio_control[localLabel] = observedNEvents_control[localLabel]/expectedNEvents_qcd[globalLabel]
    observedNEvents[globalLabel] = observedNEvents_control[localLabel]

# Next fill in observedNEvents for the signal regions; for a blind analysis, fake the observed data assuming that the ratio of observed to expected NEvents is the same in signal regions as in the control region
for signalType in signalTypesToUse:
    ...
    if (inputArguments.runUnblinded): observedNEventsLocal = get_dict_nEvents(inputPath=inputDataFilePaths[signalType]["observations"], localSignalLabels=localSignalBinLabels, inputPrefix="observedNEvents")
        ...
        for localLabel in localSignalBinLabels:
            globalLabel = dict_localToGlobalBinLabels[signalType][localLabel]
            observedNEventsLocal[localLabel] = expectedNEvents_qcd[globalLabel]*observedToExpectedRatio_control[localLabel]
    for localLabel in localSignalBinLabels:
        globalLabel = dict_localToGlobalBinLabels[signalType][localLabel]
